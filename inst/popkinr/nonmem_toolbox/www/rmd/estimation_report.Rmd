...
```{r base info}
base_info_df <- data_frame(ID = run$info$run_id,
...
est_df <- data_frame(`ESTIMATION STEP` = sprintf("%s of %s", n_estimation, length(run$estimations)),
                     METHOD = est$title,
                     ...
                     `MAXIMUM CORRELATION` = signif(est$max_correlation, 3),
                     OBJ = est$final_ofv,
                     AIC = est$aic,
                     BIC = est$bic)
